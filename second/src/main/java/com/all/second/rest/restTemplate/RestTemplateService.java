...
public class RestTemplateService implements InitializingBean {
	...
	public CommonResponseModel<String> deleteData(CommonRequestModel request, String user) {
		...
		if(StringUtils.isBlank(user)) {
			responseEntity = restTemplate.exchange(Constants.first_target_url, HttpMethod.DELETE, entity, String.class);
		}
		else {
			responseEntity = restTemplate.exchange(Constants.first_target_url + "/{user}", HttpMethod.DELETE, entity, String.class, user);
		}
