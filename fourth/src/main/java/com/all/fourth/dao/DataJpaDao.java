...
public interface DataJpaDao extends JpaRepository<DataView, BigDecimal> {
	...
	@Transactional
	@Modifying
	@Query("DELETE FROM DataView dv WHERE (:dataType IS NULL OR dv.dataType IN :dataType) AND (:enabled IS NULL OR dv.enabled = :enabled) AND (:creBy IS NULL OR dv.creBy IN :creBy) AND (:updBy IS NULL OR dv.updBy IN :updBy)")
	void deleteByConditions(@Param("dataType") List<String> dataType, @Param("enabled") Boolean enabled, @Param("creBy") List<String> createdBy, @Param("updBy") List<String> updatedBy);
}
